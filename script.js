...
victoryArr.push([numbOfLine, numbOfcells]);
...
winSum = winSum * (betNumber.innerHTML / chosenLine);
document.querySelector('#lines-word').innerHTML = "PRIZE";
document.querySelector('#lines-number').innerHTML = winSum;
}
...
function drawLines () {
		let timeWait = 300;
if (victoryArr[0][0]) {
	setTimeout(()=>document.querySelector(victoryArr[0][0]).style.visibility = 'visible', timeWait);
	timeWait+=300;
} 
 if (victoryArr[1][0]) {
	setTimeout(()=>document.querySelector(victoryArr[1][0]).style.visibility = 'visible', timeWait);
	timeWait+=300;
} 
if (victoryArr[2][0]) {
		setTimeout(()=>document.querySelector(victoryArr[2][0]).style.visibility = 'visible', timeWait);
	timeWait+=300;
}
bigButton.value = 'TAKE';
victoryArr = [];
